...
class BubbleDetector:
    ...
    def _evaluate_bubble(self, image: np.ndarray, bubble: BubbleRegion) -> Tuple[bool, float]:
        """
        Evaluate whether a bubble is marked.
        ...
        # Calculate filled area ratio
        total_pixels = thresh.size
        filled_pixels = np.sum(thresh == 0)  # Black pixels (filled)
        filled_ratio = filled_pixels / total_pixels
        
        # Simple threshold-based evaluation
        threshold = 0.15  # 15% of bubble should be filled
        is_marked = filled_ratio > threshold
        confidence = min(filled_ratio / threshold, 1.0) if is_marked else 1.0 - (filled_ratio / threshold)
        
        # Use ML classifier if available and confidence is low
        if self.ml_classifier and confidence < 0.8:
            ml_result = self._ml_classify_bubble(bubble_roi)
            if ml_result is not None:
                is_marked = ml_result
                confidence = 0.9  # High confidence for ML result
